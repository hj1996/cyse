...
def outage(provider):
	try:
		...
		location_DataFrame.to_csv(os.path.expanduser("~/Documents/senior_project/data/"+"outage-report-"+provider+"_"+str(time.time())+".csv"),index=False) #save the data to a csv
...
f=open(os.path.expanduser("~/Documents/senior_project/providers.txt"),"r") #opens the file with the list of providers
date=time.time()
data=f.readlines()
table_list=[]
for provider in data:
	table_list.append(outage(provider))
outage_report_curent_data=pd.concat(table_list)#final table to return to powerbi
