...
def istheservicedown(provider):
	try:
		...
		tables.to_csv(os.path.expanduser("~/Documents/senior_project/data/"+"istheservicedown-"+provider+"_"+str(date)+".csv"),index=False)
		print tables
		return tables
	except IndexError:
		print provider
...
for provider in data:
	table_list.append(istheservicedown(provider))
	print provider
istheservicedown_currentdata=pd.concat(table_list)